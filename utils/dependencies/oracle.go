...
type OracleBinaryVersionInfo struct {
	...
	RngEvmRandomService string `firestore:"rng-evm-random-service"`
}
...
func InstallOracleBinary(ctx context.Context, config BinaryInstallConfig, oracleType string) error {
...
func InstallRngServiceBinary(
	ctx context.Context,
	config BinaryInstallConfig,
	oracleType string,
) error {
	// Build the artifact path
	arch := runtime.GOARCH
	...
	// TODO: this is identical with the InstallOracleBinary, there should be a generic binary installer
	// which is configurable
	fullPath := fmt.Sprintf("%s/%s/%s/v%s/random-service-%s",
		oracleType,         // package name (price or rng)
		config.RollappType, // chain engine (evm or wasm)
		goos,               // linux
		config.Version,     // version
